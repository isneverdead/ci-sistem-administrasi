...
<div class="w-full shadow-lg rounded-lg px-5 pt-5 flex flex-col h-full bg-white mb-4 overflow-y-scroll">
  <div class="flex w-full justify-center">
    <h1 class="fonst-sans font-bold text-2xl text-gray-800 mb-5">Edit barang</h1>
  </div>
  <form id="formBarang" action="<?= base_url('/dashboard/inventaris/edit') ?>/<?= $barang['id'] ?>/save" method="post">
    <!-- error message -->
    ...
    <div class="w-full flex flex-row border-2 rounded-lg border-green-200">
      ...
      <div class="w-full overflow-hidden rounded-lg px-2 py-1">
        <input id="name" class="w-full focus:outline-none" type="text" placeholder="Masukkan nama barang" name="name" value="<?= $barang['name'] ?>" />
      </div>
    </div>
    ...
    <div class="w-full flex flex-row border-2 rounded-lg border-green-200">
      ...
      <div class="w-full overflow-hidden rounded-lg px-2 py-1">
        <input id="jumlah" class="w-full focus:outline-none" type="text" placeholder="Masukkan jumlah barang" name="jumlah" value="<?= $barang['jumlah'] ?>" />
      </div>
    </div>
    ...
    <div class="w-full flex flex-row border-2 rounded-lg border-green-200 ">
      ...
      <div class="w-full overflow-hidden rounded-lg px-2 py-1">
        <select class="w-full" name="status" id="barang" value="<?= $barang['status']  ?>">
    ...
    <div class="w-full flex flex-row border-2 rounded-lg border-green-200 mb-5">
      ...
      <div class="w-full overflow-hidden rounded-lg px-2 py-1">
        <input id="dipinjam_oleh" class="w-full focus:outline-none" type="text" placeholder="Dipinjam oleh siapa?" name="dipinjam_oleh" value="<?= $barang['dipinjam_oleh'] ?>" />
      </div>
    </div>
  ...
  <div class="w-full flex justify-end mb-10">
    <a href="<?= base_url('/dashboard/inventaris/delete') ?>/<?= $barang['id'] ?>">
      <button class="px-5 py-2 rounded-lg font-sans font-semibold text-gray-800 text-xl bg-red-200">Hapus</button>
    </a>
    <button type="submit" form="formBarang" class="ml-10 px-5 py-2 rounded-lg font-sans font-semibold text-gray-800 text-xl bg-green-200">Simpan</button>
  </div>
</div>