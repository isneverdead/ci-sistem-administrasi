...
<div class="w-full shadow-lg rounded-lg px-5 pt-5 flex flex-col h-full bg-white mb-4 overflow-y-scroll">
  <div class="flex w-full justify-center">
    <h1 class="fonst-sans font-bold text-2xl text-gray-800 mb-5">Edit siswa</h1>
  </div>
  <form action="<?= base_url('/dashboard/pelajar/edit') ?>/<?= $siswa['id'] ?>/save" method="post">
    <!-- error message -->
    ...
    <div class="w-full flex flex-row border-2 rounded-lg border-green-200">
      ...
      <div class="w-full overflow-hidden rounded-lg px-2 py-1">
        <input id="name" class="w-full focus:outline-none" type="text" placeholder="Masukkan nama siswa" name="name" value="<?= $siswa['nama']  ?>" />
      </div>
    </div>
    ...
    <div class="w-full flex flex-row border-2 rounded-lg border-green-200 ">
      ...
      <div class="w-full overflow-hidden rounded-lg px-2 py-1">
        <select class="w-full" name="tingkat" id="tingkat" value="<?= $siswa['tingkat']  ?>">
          <option  <?php echo ($siswa['tingkat'] == 'SD') ? 'selected' : '' ?>  value="SD">SD</option>
          <option  <?php echo ($siswa['tingkat'] == 'SMP') ? 'selected' : '' ?>  value="SMP">SMP</option>
          <option  <?php echo ($siswa['tingkat'] == 'SMA') ? 'selected' : '' ?>  value="SMA">SMA</option>
          <option  <?php echo ($siswa['tingkat'] == 'SMK') ? 'selected' : '' ?>  value="SMK">SMK</option>
          <option value="-">-</option>
        </select>
      </div>
    </div>
